      ...
      {others?.length > 0 && (
        <div style={{marginBottom:'18px'}}>
          ...
          <div style={{display:'flex', gap:'8px', flexWrap:'wrap'}}>
            {others.map((o) => (
              <Link key={o.slug} href={`${base}/${o.slug}`} style={{padding:'7px 13px', borderRadius:'9999px', background:'var(--surface-sunken)', color:'var(--text-body)', fontSize:'12.5px', fontWeight:'600', textDecoration:'none'}}>{o.name}</Link>
            ))}
          </div>
        </div>
      )}
